(() => {
  ...
  const elements = {
    ...
    sourceSynced: document.querySelector("#sourceSynced"),
    sourceValidation: document.querySelector("#sourceValidation"),
    sourceValidationMessage: document.querySelector("#sourceValidationMessage"),
  ...
  async function refresh(message) {
    ...
    try {
      ...
      state = nextState;
      selected = null;
      lastProjectionAt = state.watch?.lastSyncAt ?? new Date().toISOString();
  ...
  function renderWatchState(watchInfo) {
    if (!watchInfo?.error) {
      elements.sourceValidation.hidden = true;
      elements.sourceValidationMessage.textContent = "";
      return;
    }
    ...
    elements.sourceValidation.hidden = false;
    elements.sourceValidationMessage.textContent =
      "Invalid Ticket YAML · showing last valid snapshot";
    elements.sourceValidationMessage.title = watchInfo.error;
  }

  function renderProjectionTime() {
    elements.sourceSynced.textContent = lastProjectionAt
      ? formatInstant(lastProjectionAt)
      : "—";
    if (lastProjectionAt) {
      elements.sourceSynced.title = lastProjectionAt;
    }
  }
  ...
  function startWatchPolling() {
    ...
    watchPollTimer = window.setInterval(async () => {
      try {
        ...
        lastProjectionAt = nextState.watch?.lastSyncAt ?? lastProjectionAt;
        renderProjectionTime();
