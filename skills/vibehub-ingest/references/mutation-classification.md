...
- **contradiction**: propositions cannot both govern the same scope/time. Create
  a clearly scoped active Spec plus `conflicts_with` only when the contradiction
  itself is evidence-backed; never amend one side away.
- **supersession**: evidence explicitly says NEW replaces OLD, not merely differs
  or is newer. Keep `OLD -> NEW`; the replacement must already be an active
  canonical Spec.
...
two valid claims. If the source does not authorize a mutation, preserve the
evidence and route the missing authority explicitly; do not create a draft
approval queue.
