...
import type { MapSnapshot, Task, Territory } from "@vibehub/core/contracts";
...
import { SCRIM_TIP } from "../src/conflict-derive";
import {
...
import { InstallScreen } from "../src/components/InstallScreen";
...
export interface AppProps {
  fixtures: Record<string, MapSnapshot>;
   ...
   * MapSnapshots through the pure rollup; `1` → busy; unknown → map.
   */
...
export function App({
  ...
  const [panel, setPanel] = useState<TaskPanelSnapshot | null>(() =>
  ...
  const [conflict, setConflict] = useState<ConflictCardSnapshot | null>(() =>
  ...
  if (installName && installFixture) {
    ...
    return (
      <InstallScreen
        snapshot={installFixture}
        resolveTaskPanel={panelForTask}
  ...
  return (
    <div className="window">
      <Titlebar
        snapshot={fixture}
        snapshotNames={showSwitcher ? Object.keys(fixtures) : []}
        activeSnapshot={fixtureName}
        onSnapshotChange={switchFixture}
