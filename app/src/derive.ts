/**
 * derive.ts — pure view-model derivations from a MapSnapshot.
 *
 * HARD RULE (task brief): zero hardcoded CONTENT in JSX. Everything the
 * screen shows is either snapshot data or a mechanical join of snapshot data
 ...
 * no Date/timezone involvement, so previews render identically everywhere.
 */
...
export function taskAge(task: Task, fx: MapSnapshot): string {
 ...
 */
export function groupTasks(fx: MapSnapshot): TaskGroup[] {
...
function scopeTargetName(s: ScopeDeclaration, fx: MapSnapshot): string {
...
function scopeChip(s: ScopeDeclaration, fx: MapSnapshot): ChipView {
...
export function taskChips(task: Task, fx: MapSnapshot): ChipView[] {
...
export function titlebarStats(fx: MapSnapshot): StatView[] {
...
export function freshness(fx: MapSnapshot): FreshView {
...
function taskById(fx: MapSnapshot, id: string): Task | undefined {
 ...
 */
function occupancyOf(fx: MapSnapshot, territoryId: string): TerritoryOccupancy {
...
function conflictsIn(fx: MapSnapshot, territoryId: string): Conflict[] {
...
function subViews(fx: MapSnapshot, terr: Territory): SubView[] {
  return terr.subBlocks.map((sub) => {
    ...
    const style = terr.subBlockLayout?.[sub.id] ?? {};
    if (conflict) {
      return {
        sub,
        kind: "clash" as const,
...
export function territoryView(terr: Territory, fx: MapSnapshot): TerritoryView {
  ...
  const compact =
    !!terr.layout &&
    (terr.layout.width < COMPACT_W_PCT || terr.layout.height < COMPACT_H_PCT);
...
export function highlightForTask(task: Task, fx: MapSnapshot): Highlight {
 ...
 */
export function highlightForTerritory(terr: Territory, fx: MapSnapshot): Highlight {
...
export function highlightForLegend(kind: LegendKind, fx: MapSnapshot): Highlight {
  const litIds = new Set<string>();
