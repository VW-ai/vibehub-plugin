/**
 * Menubar-summary view types (m5, S3) — a PURE ROLLUP over MapSnapshot.
 *
 * There is deliberately NO menubar snapshot shape: the dropdown is the same
 * repo snapshot the map renders, summarized. Every field below is derived by
 * `deriveMenubar` (menubar-derive.ts) from MapSnapshot alone — tasks, conflicts
...
export interface MenubarSummary {
   ...
   * consistent with every age in the dropdown — deterministic previews).
   */
  clockText: string;
}
