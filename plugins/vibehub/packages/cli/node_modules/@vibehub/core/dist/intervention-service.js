...
import { isConflictPairIgnored, persistIgnoredConflictPair, resolveConflictPair, } from "./conflict-ignore.js";
...
function interventionHash(intervention) {
    return crypto.createHash("sha256").update(JSON.stringify(canonicalize(intervention))).digest("hex");
}
function conflictRow(db, repoId, id) {
    return db.prepare(`SELECT id, repo_id AS repoId, task_a AS taskA, task_b AS taskB,
            resolved_at AS resolvedAt, ignored
     FROM conflicts WHERE repo_id = ? AND id = ?`).get(repoId, id) ?? null;
}
/**
 ...
 */
export function applyIntervention(db, repoId, input, now) {
    if (input.requestId.trim() === "")
        throw new Error("requestId is required");
    const inputHash = interventionHash(input.intervention);
    const run = db.transaction(() => {
        const prior = db.prepare(`SELECT input_hash AS inputHash, result FROM intervention_requests WHERE repo_id = ? AND request_id = ?`).get(repoId, input.requestId);
        if (prior) {
            ...
            const receipt = JSON.parse(prior.result);
        ...
        if (intervention.kind === "generate_diagnosis") {
            ...
            if (!task || task.repoId !== repoId)
                throw new InterventionTargetNotFoundError("task not found in repository");
            if (task.state === "done") {
                ...
                const injectionId = enqueueInjection(db, repoId, task.id, intervention.kind, intervention.text, now, intervention.contextLocus);
            ...
            if (intervention.kind === "ignore_pair") {
                ...
                if (!conflict)
                    throw new InterventionTargetNotFoundError("rich conflict not found in repository");
                ...
                if (conflict.resolvedAt || conflict.ignored) {
                    ...
                    if (tasks.some((task) => task.state === "done")) {
                        ...
                        const injectionIds = taskIds.map((taskId) => enqueueInjection(db, repoId, taskId, "inject", intervention.text, now, intervention.contextLocus));
        ...
        db.prepare(`INSERT INTO intervention_requests (request_id, repo_id, input_hash, result, created_at)
       VALUES (?, ?, ?, ?, ?)`).run(input.requestId, repoId, inputHash, JSON.stringify(receipt), now);
        return receipt;
    });
    return run.immediate();
}
function appendHistory(db, repoId, taskId, action, text, at, requestId) {
