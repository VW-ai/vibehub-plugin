...
const boundedString = (maxLength) => z.string()
    .check(z.custom(value => typeof value === "string" && [...value].length <= maxLength, { message: `must contain at most ${maxLength} Unicode characters` }))
    .meta({ maxLength });
const canonicalString = (maxLength) => boundedString(maxLength).min(1).regex(/^(?!\s)[\s\S]*\S$(?![\s\S])/);
