 ...
 */
export const taskIdForBranch = (repoId, branch) => `task:${crypto.createHash("sha256").update(`${repoId}\0${branch}`).digest("hex").slice(0, 32)}`;
export function upsertTask(db, t) {
    db.prepare(`INSERT INTO tasks (id, repo_id, title, state, signal_tier, branch, worktree_path,
     ...
     ON CONFLICT(id) DO UPDATE SET
       ...
       start_head_sha = COALESCE(tasks.start_head_sha, excluded.start_head_sha)`).run(t);
}
export function readTask(db, id) {
    const r = db
        .prepare(`SELECT id, repo_id AS repoId, title, state, signal_tier AS signalTier,
 ...
 */
export function readTaskForBranch(db, repoId, branch) {
    ...
    const r = db
       ...
       LIMIT 1`)
        .get(repoId, branch, canonicalId);
    return r ?? null;
}
export function listTasks(db, repoId) {
    return db
        .prepare(`SELECT id, repo_id AS repoId, title, state, signal_tier AS signalTier,
              branch, worktree_path AS worktreePath, pr_number AS prNumber,
              pr_state AS prState, state_since AS stateSince,
...
export function saveTaskReport(db, taskId, report) {
    db.prepare(`INSERT INTO task_reports (task_id, status, done, reported_at)
     VALUES (?, ?, ?, ?)
     ON CONFLICT(task_id) DO UPDATE SET
       status = excluded.status, done = excluded.done, reported_at = excluded.reported_at`).run(taskId, report.status, report.done, report.reportedAt);
    return report;
}
export function readTaskReport(db, taskId) {
    return db.prepare(`SELECT status, done, reported_at AS reportedAt FROM task_reports WHERE task_id = ?`).get(taskId) ?? null;
}
export function upsertSession(db, s) {
    db.prepare(`INSERT INTO sessions (id, repo_id, task_id, agent, transcript_path, started_at, ended_at, end_reason)
     ...
     ON CONFLICT(id) DO UPDATE SET
       ...
       ended_at = excluded.ended_at, end_reason = excluded.end_reason`).run(s);
}
/**
 ...
 */
export function sessionIdentity(db, taskId, sessionId) {
    const rows = db
        .prepare(`SELECT id, agent, ended_at AS endedAt, end_reason AS endReason
 ...
 */
export function appendEvent(db, repoId, taskId, sessionId, event) {
    db.prepare(`INSERT INTO events (repo_id, task_id, session_id, type, at, payload)
     VALUES (?, ?, ?, ?, ?, ?)`).run(repoId, taskId, sessionId, event.type, event.at, JSON.stringify(event));
}
/**
 ...
 */
export function hasEvent(db, taskId, type) {
    return (db
 ...
 */
export function lastHookEventAt(db, repoId) {
    const r = db
...
export function readTimeline(db, taskId) {
    const rows = db
        .prepare(`SELECT payload FROM events WHERE task_id = ? ORDER BY at, id`)
        .all(taskId);
    return rows.map((r) => JSON.parse(r.payload));
}
export function addFootprint(db, repoId, f) {
    db.prepare(`INSERT INTO footprints (repo_id, task_id, session_id, path, action, at)
     VALUES (?, ?, ?, ?, ?, ?)`).run(repoId, f.taskId, f.sessionId, f.path, f.action, f.at);
}
/** Distinct files the task has EDITED — counted in SQL, never materialized. */
export function distinctEditedFileCount(db, taskId) {
    const r = db
        .prepare(`SELECT COUNT(DISTINCT path) AS n FROM footprints
       WHERE task_id = ? AND action = 'edit'`)
        .get(taskId);
    return r.n;
}
export function readFootprints(db, taskId) {
    return db
        .prepare(`SELECT task_id AS taskId, session_id AS sessionId, path, action, at
       FROM footprints WHERE task_id = ? ORDER BY at, id`)
        .all(taskId);
}
/* ── scopes ─────────────────────────────────────────────────────────────── */
/** Replace the task's declarations (seq = declaration order, contract). */
export function setScopes(db, repoId, taskId, scopes) {
    const tx = db.transaction(() => {
        db.prepare(`DELETE FROM scopes WHERE task_id = ?`).run(taskId);
        const ins = db.prepare(`INSERT INTO scopes (repo_id, task_id, seq, mode, territory_id, sub_block_id, label)
       VALUES (?, ?, ?, ?, ?, ?, ?)`);
        scopes.forEach((s, i) => ins.run(repoId, taskId, i, s.mode, s.territoryId, s.subBlockId ?? null, s.label));
    });
    tx();
}
/**
 ...
 */
export function readScopes(db, taskId) {
    const rows = db
        .prepare(`SELECT mode, territory_id AS territoryId, sub_block_id AS subBlockId, label
       FROM scopes WHERE task_id = ? ORDER BY seq`)
        .all(taskId);
    const touched = db.prepare(`SELECT COUNT(DISTINCT f.path) AS n
     FROM footprints f
    ...
    return rows.map((r) => {
        const n = touched.get(taskId, r.subBlockId ?? r.territoryId).n;
        return {
            mode: r.mode,
...
/* ── injections ─────────────────────────────────────────────────────────── */
export function enqueueInjection(db, repoId, taskId, mode, text, now, context) {
    const r = db
        .prepare(`INSERT INTO injections (repo_id, task_id, mode, text, context, created_at)
       VALUES (?, ?, ?, ?, ?, ?)`)
 ...
 */
export function claimPendingInjections(db, taskId, now) {
    return db
        .prepare(`UPDATE injections SET claimed_at = ?
 ...
 */
export function pendingInjections(db, taskId) {
    return db
        .prepare(`SELECT id, mode, text, context, created_at AS createdAt FROM injections
       WHERE claimed_at IS NULL AND task_id = ?
...
export function insertConflict(db, repoId, c, symbolFiles) {
    if (symbolFiles.length !== c.sharedSymbols.length) {
        throw new Error(`symbolFiles must align 1:1 with sharedSymbols (${symbolFiles.length} vs ${c.sharedSymbols.length})`);
    }
    const tx = db.transaction(() => {
        db.prepare(`INSERT INTO conflicts (id, repo_id, task_a, task_b, territory_id, sub_block_id, severity, detected_at)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?)`).run(c.id, repoId, c.taskIds[0], c.taskIds[1], c.territoryId, c.subBlockId ?? null, c.severity, c.detectedAt);
        const ins = db.prepare(`INSERT INTO conflict_symbols (conflict_id, seq, name, file) VALUES (?, ?, ?, ?)`);
...
export function readConflict(db, id) {
    const r = db
        .prepare(`SELECT id, repo_id AS repoId, task_a, task_b, territory_id AS territoryId,
    ...
    if (!r)
        return null;
    if (isConflictPairIgnored(db, r.repoId, [r.task_a, r.task_b]))
        return null;
    const symbols = db
...
export function listActiveConflicts(db, repoId) {
    const ids = db.prepare(`SELECT id FROM conflicts
     WHERE repo_id = ? AND resolved_at IS NULL AND ignored = 0
     ORDER BY detected_at, id`).all(repoId);
    return ids.map((row) => readConflict(db, row.id)).filter((value) => value !== null);
}
export function saveDiagnosis(db, conflictId, d) {
    db.prepare(`INSERT INTO conflict_diagnoses (conflict_id, verdict, sides, suggested, diagnosed_at, engine)
     VALUES (?, ?, ?, ?, ?, ?)
     ON CONFLICT(conflict_id) DO UPDATE SET
       verdict = excluded.verdict, sides = excluded.sides,
       suggested = excluded.suggested, diagnosed_at = excluded.diagnosed_at,
       engine = excluded.engine`).run(conflictId, d.verdict, JSON.stringify(d.sides), d.suggested, d.provenance.diagnosedAt, d.provenance.engine);
}
/**
 ...
 */
export function readDiagnosis(db, conflictId) {
    const r = db
        .prepare(`SELECT verdict, sides, suggested, diagnosed_at AS diagnosedAt, engine
       FROM conflict_diagnoses WHERE conflict_id = ?`)
        .get(conflictId);
    if (!r)
        return null;
    const stale = db
        .prepare(`SELECT COUNT(*) AS n FROM footprints f
...
/* ── notifications ──────────────────────────────────────────────────────── */
export function recordNotification(db, repoId, kind, refId, now) {
    db.prepare(`INSERT INTO notifications (repo_id, kind, ref_id, created_at) VALUES (?, ?, ?, ?)`).run(repoId, kind, refId, now);
}
/** Ledger count since a timestamp — the base fact for the 通知预算 (020). */
export function notificationCountSince(db, repoId, sinceIso) {
    const r = db
        .prepare(`SELECT COUNT(*) AS n FROM notifications WHERE repo_id = ? AND created_at >= ?`)
        .get(repoId, sinceIso);
    return r.n;
}
