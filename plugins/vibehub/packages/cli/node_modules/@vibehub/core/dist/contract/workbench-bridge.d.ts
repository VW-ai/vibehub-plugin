...
export type WorkbenchBridgeErrorStatus = "db_missing" | "repo_uninitialized" | "unsynced" | "not_found" | "evidence_unavailable" | "idempotency_conflict" | "bridge_unavailable" | "internal_error";
...
export type WorkbenchBridgeResult<T> = {
    ...
    status: WorkbenchBridgeErrorStatus;
    message: string;
};
...
export interface WorkbenchBridge {
    getSnapshot(repo: WorkbenchRepoRef): Promise<WorkbenchBridgeResult<MapSnapshot>>;
    getTaskPanel(request: WorkbenchRepoRef & {
        taskId: string;
    }): Promise<WorkbenchBridgeResult<TaskPanelSnapshot>>;
    getConflictDetail(request: WorkbenchRepoRef & {
        conflictId: string;
    }): Promise<WorkbenchBridgeResult<ConflictCardSnapshot>>;
    applyIntervention(request: WorkbenchRepoRef & {
        requestId: string;
