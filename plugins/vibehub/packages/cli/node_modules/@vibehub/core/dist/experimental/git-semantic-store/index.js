...
import { openDb, withReadonlyDb } from "../../db.js";
...
const canonicalize = (value) => {
    ...
    if (typeof value === "number") {
        if (!Number.isFinite(value))
            throw new Error("git semantic store: non-finite number");
        return value;
    }
...
const parseStoredJson = (raw, field) => {
    try {
...
const storePathFor = (worktreeRoot) => path.join(path.resolve(worktreeRoot), STORE_RELATIVE_PATH);
const semanticDigest = (features, specs, repoProvenance) => sha256(serialize({
...
const readFeatureDocuments = (db, repoId) => db.prepare(`SELECT feature_id, created_at FROM kb_features
    WHERE repo_id = ? ORDER BY feature_id`).all(repoId).map((row) => ({
    schema_version: SCHEMA_VERSION,
    kind: "feature",
    ...row,
}));
const readProvenance = (db, repoId, specId) => {
    ...
    FROM kb_provenance_events WHERE repo_id = ? AND ${clause} ORDER BY id`).all(...params)
        .map((row) => ({ ...row, payload: parseStoredJson(row.payload, "kb_provenance_events.payload") }));
};
const readSpecDocuments = (db, repoId) => {
    const specs = db.prepare(`SELECT spec_id, feature_id, state, current_revision, source_kind,
    created_at, updated_at FROM kb_specs WHERE repo_id = ? ORDER BY spec_id`).all(repoId);
    ...
    return specs.map((spec) => {
        ...
        const revisions = revisionsStatement.all(repoId, spec.spec_id)
            .map((revision) => ({
            ...revision,
            tags: (() => {
                const value = parseStoredJson(revision.tags, "kb_spec_revisions.tags");
                if (!Array.isArray(value))
                    throw new Error("git semantic store: revision tags must be an array");
                return value.map((tag, index) => stringValue(tag, `kb_spec_revisions.tags[${index}]`));
            })(),
            evidence: evidenceStatement.all(repoId, spec.spec_id, revision.revision),
            anchors: anchorsStatement.all(repoId, spec.spec_id, revision.revision),
        }));
        ...
        return {
            ...
            revisions,
            relations: relationsStatement.all(repoId, spec.spec_id),
            provenance: readProvenance(db, repoId, spec.spec_id),
        };
    });
};
const writeDocument = (directory, document) => {
...
export function exportGitSemanticStore(options) {
    ...
    return withReadonlyDb(options.dbPath, (db) => {
        const repo = db.prepare(`SELECT slug, default_branch, created_at FROM repos WHERE id = ?`)
            .get(options.repoId);
        if (!repo)
            throw new Error(`git semantic store: repository not found: ${options.repoId}`);
        ...
        for (const document of featureDocuments)
            validateFeature(document, document.feature_id);
        for (const document of specDocuments)
            validateSpec(document, document.spec_id);
      ...
      AND NOT EXISTS (SELECT 1 FROM kb_specs s
        ...
        if (orphanProvenance)
            throw new Error("git semantic store: provenance references a missing spec");
...
const stringValue = (value, field) => {
    if (typeof value !== "string")
        throw new Error(`git semantic store: ${field} must be a string`);
    return value;
};
const nullableString = (value, field) => value === null ? null : stringValue(value, field);
const canonicalIdentity = (value, field) => {
...
const arrayValue = (value, field) => {
    if (!Array.isArray(value))
        throw new Error(`git semantic store: ${field} must be an array`);
    return value;
};
const assertSortedUnique = (values, key, field) => {
...
const readCanonicalDocument = (filePath, expectedDigest) => {
...
const validateProvenance = (value, field) => {
...
const validateEntry = (value, kind, index) => {
...
const validateFeature = (value, expectedId) => {
...
const validateAnchor = (value, field) => {
...
const validateEvidence = (value, field) => {
    const row = exactKeys(value, [
...
const validateRevision = (value, field) => {
    const row = exactKeys(value, [
    ...
    if (!SPEC_TYPES.has(type))
        throw new Error(`git semantic store: invalid spec type in ${field}`);
    return {
...
const validateSpec = (value, expectedId) => {
    ...
    assertSortedUnique(relations, (relation) => `${relation.type}\0${relation.to_spec_id}`, `${field}.relations`);
    ...
    return {
        ...
        state: (() => {
            ...
            if (!SPEC_STATES.has(state))
                throw new Error(`git semantic store: invalid state in ${field}`);
            return state;
        })(),
...
const safeDocumentPath = (storePath, relative) => {
...
const assertExactInventory = (storePath, manifest) => {
    ...
    const visit = (directory) => {
        for (const entry of fs.readdirSync(directory, { withFileTypes: true })) {
            ...
            if (entry.isDirectory())
                ...
                found.add(path.relative(storePath, absolute));
            else
                throw new Error(`git semantic store: non-regular store entry: ${absolute}`);
        }
    };
    visit(storePath);
...
const insertProvenance = (db, repoId, specId, rows) => {
    ...
    for (const row of rows) {
        insert.run(row.event_id, repoId, row.operation, specId, row.actor, row.task_id, row.request_id, row.at, serialize(row.payload).trimEnd());
    }
};
const populateDatabase = (db, manifest, features, specs, rootPath) => db.transaction(() => {
    const repoResult = db.prepare(`INSERT INTO repos (root_path, slug, default_branch, created_at)
    VALUES (?, ?, ?, ?)`).run(path.resolve(rootPath), manifest.repository.slug, manifest.repository.default_branch, manifest.repository.created_at);
    ...
    for (const feature of features)
        insertFeature.run(repoId, feature.feature_id, feature.created_at);
    ...
    for (const spec of specs) {
        insertSpec.run(repoId, spec.spec_id, spec.feature_id, spec.state, spec.current_revision, spec.created_at, spec.updated_at);
        for (const revision of spec.revisions) {
            insertRevision.run(repoId, spec.spec_id, revision.revision, revision.type, revision.summary, revision.detail, revision.priority, revision.layer, revision.domain, serialize(revision.tags).trimEnd(), revision.producer, revision.produced_at);
            for (const evidence of revision.evidence) {
                insertEvidence.run(repoId, evidence.evidence_id, spec.spec_id, revision.revision, evidence.source_type, evidence.source_ref, evidence.exact_quote, evidence.evidence_ref, evidence.content_hash, evidence.confidence, evidence.producer, evidence.produced_at);
            }
            for (const anchor of revision.anchors) {
                insertAnchor.run(repoId, spec.spec_id, revision.revision, anchor.file, anchor.symbol, anchor.line_start, anchor.line_end, anchor.content_hash);
            }
        }
    }
    ...
    for (const spec of specs) {
        for (const relation of spec.relations) {
            ...
            insertRelation.run(repoId, spec.spec_id, relation.to_spec_id, relation.type, relation.rationale, relation.created_at);
        }
    }
    ...
    for (const spec of specs)
        insertProvenance(db, repoId, spec.spec_id, spec.provenance);
    return repoId;
})();
export function importGitSemanticStore(options) {
    ...
    const features = manifest.features.map((entry) => validateFeature(readCanonicalDocument(safeDocumentPath(storePath, entry.file), entry.sha256), entry.id));
    const specs = manifest.specs.map((entry) => validateSpec(readCanonicalDocument(safeDocumentPath(storePath, entry.file), entry.sha256), entry.id));
    ...
    try {
        db = openDb(stagingDb);
        const repoId = populateDatabase(db, manifest, features, specs, options.targetRepoRootPath);
        const foreignKeyErrors = db.prepare("PRAGMA foreign_key_check").all();
        if (foreignKeyErrors.length > 0)
            throw new Error("git semantic store: imported foreign key violation");
