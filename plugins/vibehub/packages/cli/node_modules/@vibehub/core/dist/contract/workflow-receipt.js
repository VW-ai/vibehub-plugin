...
export const WORKFLOW_PHASES = ["prepare", "execute", "complete"];
...
export const WORKFLOW_VISIBILITIES = ["silent", "brief", "expanded"];
export const WORKFLOW_EFFECTS = ["read", "write", "injection", "health_check", "none"];
 ...
 */
export function validateWorkflowReceiptStructure(value) {
    const errors = [];
    if (!isRecord(value))
...
export function renderWorkflowReceiptText(receipt, options = {}) {
    const validation = validateWorkflowReceiptStructure(receipt);
    if (!validation.ok)
        throw new Error(`invalid WorkflowReceiptV1 structure: ${validation.errors.join("; ")}`);
    const requested = options.width;
    const width = Number.isFinite(requested)
        ? Math.min(160, Math.max(20, Math.floor(requested)))
        : 80;
...
function validateEvidence(value, index, errors) {
    ...
    if (value.source === "operation_result") {
        ...
        if (value.returnedCount !== undefined)
        ...
        if (typeof value.returnedCount === "number" && typeof value.totalCount === "number"
            && value.returnedCount > value.totalCount)
        ...
        if (typeof value.ok === "boolean" && typeof value.conflictCount === "number"
            && value.ok !== (value.conflictCount === 0))
...
function operationPresentation(operation) {
    return Object.prototype.hasOwnProperty.call(CANONICAL_OPERATION_PRESENTATION, operation)
        ? CANONICAL_OPERATION_PRESENTATION[operation]
        : undefined;
}
...
function section(label, raw, width, budget, marker) {
...
function labeled(label, value, width) {
...
function splitByDisplayWidth(value, width) {
    const chunks = [];
    let chunk = "";
    let columns = 0;
...
function exactKeys(value, allowed, name, errors, optional = false) {
...
function safeIds(value, name, errors, nonEmpty) {
    if (!validIds(value, nonEmpty))
        errors.push(`${name} must contain ${nonEmpty ? "non-empty " : ""}unique positive safe injection ids`);
}
