...
import { applyIntervention, CURRENT_SCHEMA_VERSION, exportTeamMapSnapshot, GitFacade, ingestCanonicalHookEvent, migrateSqliteSemanticStoreToGit, openDb, projectDoctorReceipt, projectInitReceipt, projectInjectionInterventionReceipt, readTask, renderWorkflowReceiptText, resolveDbPath, RuntimeService, OperationDispatcher, OPERATION_EXIT_CLASS, syncTeamSnapshot, vibehubHome, } from "@vibehub/core";
...
function parseFlags(argv, failureMode = "exit") {
    let dbFlag;
    const flags = { repo: process.cwd(), out: undefined, json: false };
    for (let i = 0; i < argv.length; i++) {
        const a = argv[i];
        if (a === "--repo")
            flags.repo = argv[++i] ?? flags.repo;
        else if (a === "--db")
            dbFlag = argv[++i];
        else if (a === "--out")
            flags.out = argv[++i];
...
const USAGE = `usage:
  ...
  vibehub team sync    [--repo <path>] [--db <path>] [--json]
  vibehub team snapshot [--repo <path>] [--db <path>] [--out <file>]`;
function parseKbFlags(argv) {
    let dbFlag;
    let repo = process.cwd();
    let repoId;
    let actor;
    let taskId;
    let requestId = `cli-${Date.now()}`;
    let inputText;
    let json = false;
    for (let i = 0; i < argv.length; i++) {
        const flag = argv[i];
...
function runOperation(group, operation, argv) {
    let db;
    try {
        if (!operation)
            throw new Error("KB operation is required");
        const flags = parseKbFlags(argv);
        if (!flags.json)
            throw new Error("kb operations require --json");
        db = openDb(flags.db);
        const session = GitFacade.sessionContextAt(flags.repo);
        const root = session.repoRoot;
        const row = flags.repoId ? { id: flags.repoId } : db.prepare(`SELECT id FROM repos WHERE root_path=?`).get(root);
        ...
        const result = { ok: false, error: { code: "validation_error", message: error instanceof Error ? error.message : String(error), details: null, nextSafeActions: [`Run vibehub ${group} with --json and a valid JSON --input payload.`] } };
        process.stdout.write(`${JSON.stringify(result)}\n`);
        return 2;
    }
    finally {
...
function runSemanticMigration(argv) {
    const flags = parseFlags(argv, "throw");
    if (!flags.json)
        throw new Error("kb migrate-store requires --json");
    const session = GitFacade.sessionContextAt(flags.repo);
    const db = openDb(flags.db);
    let locked = false;
    try {
        const repo = db.prepare(`SELECT id FROM repos WHERE root_path=?`).get(session.repoRoot);
        if (!repo)
            throw new Error("repository is not initialized in VibeHub; run vibehub init first");
        ...
        if (after !== before)
            throw new Error("SQLite changed while the migration backup was being created");
 ...
 */
function runHook(eventArg, rest) {
    ...
    try {
        ...
        const payloadEvent = typeof payload === "object" && payload !== null
            ...
            : undefined;
        const event = (eventArg ?? payloadEvent);
        ...
        try {
            ...
            if (result.delivery)
                console.log(JSON.stringify(projectHookOutput(host, result.delivery)));
        }
        finally {
            db.close();
        }
        ...
        try {
            ...
            fs.appendFileSync(path.join(logDir, "hook.log"), `${new Date().toISOString()} ${host}:${eventArg ?? "?"} ${String(err)}\n`);
        }
        catch {
...
export function main(argv) {
    ...
    if (group === "kb" && cmd === "migrate-store") {
        try {
            return runSemanticMigration(rest);
        }
        catch (error) {
            process.stdout.write(`${JSON.stringify({ ok: false, error: { code: "validation_error", message: error instanceof Error ? error.message : String(error) } })}\n`);
            return 2;
        }
    }
    if (group === "kb" || group === "distill")
    ...
    if (group === "inject" && cmd) {
        ...
        try {
            ...
            const result = applyIntervention(db, task.repoId, { requestId, intervention }, new Date().toISOString());
            if (json)
                console.log(JSON.stringify(result));
            else
                console.log(renderReceipt(projectInjectionInterventionReceipt({
    ...
    const topLevelFlags = () => parseFlags([cmd, ...rest].filter((value) => value !== undefined));
    if (group === "setup") {
        ...
        if (flags.json)
            ...
            console.log(`${result.command}: ${result.outcome}`);
    ...
    if (group === "init") {
        ...
        const result = new RuntimeService({ dbPath: flags.db }).initialize(flags.repo, stateDirFor(flags.db), releaseAssetRoot(), releaseAssetManifest());
        if (flags.json)
            console.log(JSON.stringify(result, null, 2));
        else
            console.log(renderReceipt(projectInitReceipt({
    ...
    if (group === "doctor") {
        const flags = topLevelFlags();
        const result = new RuntimeService({ dbPath: flags.db }).doctor(flags.repo, stateDirFor(flags.db), releaseAssetRoot(), releaseAssetManifest());
        if (flags.json)
            console.log(JSON.stringify(result, null, 2));
        else
            console.log(renderReceipt(projectDoctorReceipt({
    ...
    if (cmd === "sync") {
        ...
        if (flags.json) {
            ...
            const fetch = result.fetchOk === null
                ? "fetch skipped"
                : result.fetchOk
                    ? "fetched"
                    : "FETCH FAILED — snapshot is stale";
            console.log(`${result.repoRoot}: ${fetch}; ${result.branches} branches ` +
