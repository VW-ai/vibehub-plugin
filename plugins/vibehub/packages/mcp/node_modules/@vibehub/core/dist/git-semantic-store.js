...
import { exportGitSemanticStore as exportLegacyGitSemanticStore, importGitSemanticStore as importLegacyGitSemanticStore, } from "./experimental/git-semantic-store/index.js";
...
const runGit = (repoRoot, args) => execFileSync("git", args, {
    cwd: repoRoot,
    encoding: "utf8",
    env: {
        ...process.env,
...
const resolveCommit = (repoRoot, ref) => {
    ...
    if (!/^[0-9a-f]{40}$/.test(commit))
        throw new Error(`git semantic store: invalid commit for ${ref}`);
    return commit;
};
const show = (repoRoot, commit, file) => runGit(repoRoot, ["show", `${commit}:${file}`]);
const stableFile = (kind, id) => `${kind}/sha256-${sha256(id)}.yaml`;
export const stableSemanticPath = (kind, id) => `${STORE_RELATIVE_PATH}/${stableFile(kind, id)}`;
const provenanceBody = (scope, event) => {
    const { event_id: _ignored, durable_id: _existing, ...body } = event;
    return { scope, ...body };
};
export const durableProvenanceId = (scope, event) => sha256(serialize(provenanceBody(scope, event)));
const toDurableProvenance = (scope, event) => {
...
const fromDurableProvenance = (event, eventId) => {
    const { durable_id: _ignored, ...body } = event;
    return { event_id: eventId, ...body };
};
const transformSpecToCanonical = (value) => {
    ...
    const provenance = spec.provenance.map((event) => toDurableProvenance(specId, bag(event, "spec.provenance event")));
    provenance.sort((left, right) => left.at < right.at ? -1 :
        left.at > right.at ? 1 :
...
const transformFeatureToCanonical = (value) => ({
    ...bag(value, "feature"),
    schema_version: 2,
});
...
const inventoryDigest = (files) => sha256(serialize(files.slice()
...
export function exportGitSemanticStore(options) {
    ...
    try {
        ...
        const manifest = parseCanonical(fs.readFileSync(path.join(exported.storePath, "manifest.yaml"), "utf8"), "legacy manifest");
        const protocol = {
            schema_version: 2,
            format: FORMAT,
        ...
        return {
            ...
            provenanceCount: manifest.repo_provenance.length +
                manifest.specs.reduce((total, entry) => {
                    const document = parseCanonical(fs.readFileSync(path.join(exported.storePath, entry.file), "utf8"), entry.file);
...
const validateRefSpec = (value, expectedId, label) => {
    ...
    if (!Array.isArray(spec.provenance))
        throw new Error(`git semantic store: missing provenance: ${label}`);
...
const repoProvenanceFromDocument = (document, label) => {
...
const validateProtocol = (value, label) => {
...
const readProtocolAtCommit = (repoRoot, commit) => {
...
export function readSpecAtRef(repoRoot, ref, specId) {
...
export function diffSemanticRefs(repoRoot, baseRef, targetRef) {
...
const readRefInventory = (repoRoot, commit) => {
...
const regularFiles = (directory, root = directory) => {
    const files = [];
    for (const entry of fs.readdirSync(directory, { withFileTypes: true })) {
        ...
        if (entry.isDirectory())
            files.push(...regularFiles(absolute, root));
...
const readWorktreeInventory = (repoRootInput) => {
...
export function hasGitSemanticStore(repoRoot) {
    return fs.statSync(path.join(path.resolve(repoRoot), STORE_RELATIVE_PATH, "protocol.yaml"), {
        throwIfNoEntry: false,
    })?.isFile() === true;
}
export function inspectGitSemanticStoreWorktree(repoRoot) {
    ...
    return {
        ...
        provenanceCount: inventory.files.filter((file) => file.path.startsWith("provenance/")).length +
            inventory.files.filter((file) => file.path.startsWith("specs/"))
                .reduce((total, file) => total + (file.document.provenance?.length ?? 0), 0),
    };
}
const buildLegacyAdapter = (inventory, worktree) => {
    ...
    for (const file of inventory.files) {
        if (file.path.startsWith("features/")) {
            const document = { ...file.document, schema_version: 1 };
            const bytes = serialize(document);
            ...
            if (!Array.isArray(rawEvents))
                throw new Error(`git semantic store: invalid provenance: ${specId}`);
    ...
    provenance.sort((left, right) => left.event.at < right.event.at ? -1 :
    ...
    for (const file of inventory.files.filter((item) => item.path.startsWith("specs/"))) {
        ...
        const document = {
            ...
            provenance: rawEvents.slice()
                ...
                .map((event) => fromDurableProvenance(event, eventIds.get(event.durable_id))),
        };
        const bytes = serialize(document);
        const contentHash = sha256(bytes);
...
export function materializeSemanticCacheAtRef(options) {
    ...
    if (fs.existsSync(dbPath)) {
        ...
        const repoId = withReadonlyDb(dbPath, (db) => db.prepare("SELECT id FROM repos ORDER BY id LIMIT 1").get().id);
        return {
            ref: options.ref,
            commit,
            semanticDigest: inventory.semanticDigest,
