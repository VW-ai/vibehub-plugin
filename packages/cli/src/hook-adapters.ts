...
export type AdaptedHookInput =
  ...
  | { kind: "ignored"; reason: "codex_subagent_event" };
...
function assertEvent(payload: WireRecord, eventName: HookEventName): void {
...
function directToolTouches(payload: WireRecord): CanonicalToolTouch[] {
  const toolName = optionalString(payload["tool_name"]);
  const input = payload["tool_input"];
  if (!toolName || typeof input !== "object" || input === null || Array.isArray(input)) return [];
  const file = optionalString((input as WireRecord)["file_path"]);
  if (!file) return [];
 ...
 */
export function codexApplyPatchTouches(command: string): CanonicalToolTouch[] {
  const paths: string[] = [];
  for (const line of command.split(/\r?\n/)) {
    const match = /^\*\*\* (?:Add File|Delete File|Update File|Move to): (.+)$/.exec(line);
    if (match?.[1]) paths.push(match[1]);
  }
  return [...new Set(paths)].map((path) => ({ action: "edit" as const, path }));
}

function codexToolTouches(payload: WireRecord): CanonicalToolTouch[] {
  if (payload["tool_name"] !== "apply_patch") return directToolTouches(payload);
  const input = payload["tool_input"];
  const command = typeof input === "string"
    ? optionalString(input)
    : typeof input === "object" && input !== null && !Array.isArray(input)
      ? optionalString((input as WireRecord)["command"])
        ?? optionalString((input as WireRecord)["patch"])
        ?? optionalString((input as WireRecord)["input"])
      : undefined;
  return command ? codexApplyPatchTouches(command) : [];
}

function sharedEventFields(
  host: HookHost,
  eventName: HookEventName,
  payload: WireRecord,
): Pick<
  CanonicalHookEvent,
  "host" | "eventName" | "sessionId" | "cwd" | "transcriptPath"
> {
...
export function adaptCodexHook(
  ...
  const payload = record(raw);
  if (
    (eventName === "UserPromptSubmit" || eventName === "PostToolUse")
  ...
  const turnScoped = new Set<HookEventName>([
    "UserPromptSubmit",
    "PostToolUse",
    "Stop",
    "SubagentStop",
...
export function adaptHookInput(
  host: HookHost,
  eventName: HookEventName,
  raw: unknown,
): AdaptedHookInput {
...
export function projectClaudeCodeHookOutput(delivery: HookDeliveryDirective): HookWireOutput {
...
export function projectCodexHookOutput(delivery: HookDeliveryDirective): HookWireOutput {
...
export function projectHookOutput(
  host: HookHost,
