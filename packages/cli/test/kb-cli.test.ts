...
import {DistillationService,enqueueInjection,ingestHookEvent,openDb,pendingInjections,upsertRepo} from "@vw-ai/vibehub-core";import {main} from "../src/main.js";
...
describe("vibehub kb JSON adapter",()=>{let dir:string,repo:string,dbPath:string,commit:string;beforeEach(()=>{dir=fs.mkdtempSync(path.join(os.tmpdir(),"vh-cli-kb-"));repo=path.join(dir,"repo");fs.mkdirSync(repo);execFileSync("git",["init","-b","main"],{cwd:repo});execFileSync("git",["config","user.name","Test"],{cwd:repo});execFileSync("git",["config","user.email","test@example.com"],{cwd:repo});fs.writeFileSync(path.join(repo,"README.md"),"test\n");execFileSync("git",["add","README.md"],{cwd:repo});execFileSync("git",["commit","-m","initial"],{cwd:repo});commit=execFileSync("git",["rev-parse","HEAD"],{cwd:repo,encoding:"utf8"}).trim();dbPath=path.join(dir,"db.sqlite");const db=openDb(dbPath);upsertRepo(db,fs.realpathSync(repo),null,"main","2026-07-13T00:00:00.000Z");db.close();});afterEach(()=>{vi.restoreAllMocks();fs.rmSync(dir,{recursive:true,force:true});});
  it("keeps the real hook process fail-open for malformed flags",()=>{const cliRoot=path.resolve(".");const run=spawnSync(process.execPath,[path.join(cliRoot,"dist/main.js"),"hook","SessionStart","--bogus"],{cwd:cliRoot,input:JSON.stringify({session_id:"malformed-flags",cwd:repo}),encoding:"utf8",env:{...process.env,HOME:dir}});expect(run.status,run.error?.message??run.stderr).toBe(0);});
  it("exits zero without consuming an intervention when hook validation fails",()=>{const db=openDb(dbPath);const started=ingestHookEvent(db,"SessionStart",{session_id:"cli-session",cwd:repo},{now:()=>new Date("2026-07-13T00:00:00.000Z")});enqueueInjection(db,1,started.taskId,"inject","deliver later","2026-07-13T00:01:00.000Z");db.close();vi.spyOn(fs,"readFileSync").mockReturnValue(JSON.stringify({session_id:"cli-session",cwd:repo,tool_name:"Edit",tool_input:{file_path:path.join(dir,"outside.ts")}}));const priorHome=process.env["HOME"];process.env["HOME"]=dir;try{expect(main(["hook","PostToolUse","--db",dbPath])).toBe(0);}finally{if(priorHome===undefined)delete process.env["HOME"];else process.env["HOME"]=priorHome;}const check=openDb(dbPath);expect(pendingInjections(check,started.taskId)).toMatchObject([{text:"deliver later"}]);check.close();});
