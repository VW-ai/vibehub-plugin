...
const db=openDb(workerData.dbPath);try{parentPort.postMessage(new OperationDispatcher(db).dispatch(workerData.operation??"kb.spec.apply",workerData.context,workerData.input));}finally{db.close();}
