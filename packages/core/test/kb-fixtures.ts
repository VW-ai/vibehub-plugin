...
export function seedActiveMapping(db:Db,repoId:number,features:Array<{id:string;name:string;parentId?:string;anchors?:Array<{file:string;symbol?:string}>;layout?:{pctLeft:number;pctTop:number;pctWidth:number;pctHeight:number}}>,now:string):void{
  ...
  for(const f of features)if(f.layout)db.prepare(`INSERT INTO mapping_version_layouts(repo_id,version_id,feature_id,pct_left,pct_top,pct_width,pct_height,computed_at) VALUES(?,?,?,?,?,?,?,?)`).run(repoId,version,f.id,f.layout.pctLeft,f.layout.pctTop,f.layout.pctWidth,f.layout.pctHeight,now);
