...
describe("KnowledgeService canonical boundary",()=>{
  ...
  const spec=(id:string,summary=id)=>({id,featureId:"auth",type:"decision" as const,summary,evidence,anchors:[{file:"src/auth/session.ts",symbol:"session"}]});

  it("applies an atomic idempotent batch and rejects key hash conflicts",()=>{
    const input={idempotencyKey:"batch-1",specs:[spec("decision-a"),spec("decision-b")]};
    expect(service.applySpecBatch(1,input,ctx)).toEqual(service.applySpecBatch(1,input,ctx));
    expect(()=>service.applySpecBatch(1,{...input,specs:[spec("decision-c")]},ctx)).toThrow(/idempotency key/i);
    expect((db.prepare(`SELECT COUNT(*) AS n FROM kb_specs`).get() as {n:number}).n).toBe(2);
    expect(()=>service.applySpecBatch(1,{idempotencyKey:"bad",specs:[spec("decision-c"),{...spec("decision-d"),evidence:[]}]},ctx)).toThrow(/validation_error/);
    expect(db.prepare(`SELECT 1 FROM kb_specs WHERE spec_id='decision-c'`).get()).toBeUndefined();
    expect(()=>service.applySpecBatch(1,{idempotencyKey:" padded ",specs:[spec("decision-e")]},ctx)).toThrow(/validation_error/);
    expect(()=>service.applySpecBatch(1,{idempotencyKey:"canonical",specs:[spec(" decision-e")]},ctx)).toThrow(/validation_error/);
    expect(()=>service.applySpecBatch(1,{idempotencyKey:"canonical",specs:[{...spec("decision-e"),evidence:[{...evidence[0]!,sourceRef:"chat:1 "}]}]},ctx)).toThrow(/validation_error/);
    expect(()=>service.applySpecBatch(1,{idempotencyKey:"optional-ref",specs:[{...spec("decision-e"),evidence:[{...evidence[0]!,evidenceRef:" ref "}]}]},ctx)).toThrow(/validation_error/);
    expect(()=>service.applySpecBatch(1,{idempotencyKey:"optional-tag",specs:[{...spec("decision-e"),tags:[" tag "]}]},ctx)).toThrow(/validation_error/);
    expect(()=>service.applySpecBatch(1,{idempotencyKey:"optional-hash",specs:[{...spec("decision-e"),anchors:[{file:"src/a.ts",contentHash:" hash "}]}]},ctx)).toThrow(/validation_error/);
    expect(()=>service.applySpecBatch(1,{idempotencyKey:"path",specs:[{...spec("decision-e"),anchors:[{file:"./src/a.ts"}]}]},ctx)).toThrow(/validation_error/);
  ...
  it("enforces lifecycle, immutable amendment history and supersede replacement rules",()=>{
    service.applySpecBatch(1,{idempotencyKey:"seed",specs:[spec("old"),spec("inactive"),spec("new")]},ctx);
    ...
    service.mutate(1,"deprecate",{specId:"inactive",idempotencyKey:"d-inactive"},ctx);
    expect(()=>service.mutate(1,"supersede",{specId:"old",replacementSpecId:"inactive",idempotencyKey:"s-no"},ctx)).toThrow(/already be an active/);
    service.mutate(1,"supersede",{specId:"old",replacementSpecId:"new",idempotencyKey:"s-yes"},ctx);
  ...
  it("defaults search to active, honors directory containment and reports unplaced/review queues",()=>{
    service.applySpecBatch(1,{idempotencyKey:"seed",specs:[spec("active","database canonical"),{...spec("unplaced","unplaced"),featureId:undefined,evidence:[{...evidence[0]!,confidence:.4}]}]},ctx);
    service.mutate(1,"mark_stale",{specId:"active",idempotencyKey:"stale"},ctx);
  ...
  it("validates relation cycles, cross-repo identities, evidence and anchor ranges",()=>{
    service.applySpecBatch(1,{idempotencyKey:"seed",specs:[{...spec("a"),relations:[{toSpecId:"b",type:"depends_on"}]},spec("b")]},ctx);
    expect(()=>service.applySpecBatch(1,{idempotencyKey:"cycle",specs:[{...spec("c"),relations:[{toSpecId:"a",type:"depends_on"},{toSpecId:"c",type:"depends_on"}]}]},ctx)).toThrow(/self relation/);
    expect(()=>service.applySpecBatch(1,{idempotencyKey:"line",specs:[{...spec("bad"),anchors:[{file:"src/x.ts",lineStart:5,lineEnd:2}]}]},ctx)).toThrow(/validation_error/);
  ...
  it("mark-stale mutates only the addressed repository when spec ids collide",()=>{
    ...
    service.applySpecBatch(1,{idempotencyKey:"repo-one-seed",specs:[spec("shared-spec")]},{...ctx,requestId:"repo-one-seed"});
    service.applySpecBatch(repoTwo.id,{idempotencyKey:"repo-two-seed",specs:[spec("shared-spec")]},ctxTwo);
  ...
  it("dispatcher returns exact envelopes and explicit actor/task/error classes",()=>{
    ...
    const noTask=dispatch.dispatch("kb.spec.apply",{...ctx,taskId:undefined,requestId:"req:no-task"},{idempotencyKey:"x",specs:[spec("x")]});expect(noTask).toMatchObject({ok:false,error:{code:"task_required"}});
    const ok=dispatch.dispatch("kb.spec.apply",ctx,{idempotencyKey:"x",specs:[spec("x")]});expect(ok).toMatchObject({ok:true,meta:{operation:"kb.spec.apply",repoId:1,requestId:"req:1"}});
    expect(ok).toMatchObject({data:{operation:"spec_batch",state:"active",validation:{status:"passed"},created:["x"]}});
    expect(dispatch.dispatch("kb.spec.apply",{...ctx,requestId:"req:2"},{idempotencyKey:"other",specs:[spec("x")]})).toMatchObject({ok:false,error:{code:"already_exists"}});
    expect(dispatch.dispatch("kb.spec.apply",{...ctx,requestId:"req:3"},{idempotencyKey:"x",specs:[spec("different")]})).toMatchObject({ok:false,error:{code:"idempotency_conflict"}});
    expect(dispatch.dispatch("kb.spec.apply",{...ctx,requestId:"req:6"},{idempotencyKey:"y",specs:[spec("y")]})).toMatchObject({ok:true});
    expect(dispatch.dispatch("kb.supersede",{...ctx,requestId:"req:7"},{specId:"x",replacementSpecId:"y",idempotencyKey:"s"})).toMatchObject({ok:true,data:{state:"superseded"}});
    expect(dispatch.dispatch("kb.spec.apply",{...ctx,requestId:"req:8"},{idempotencyKey:"cycle",specs:[{...spec("z"),relations:[{toSpecId:"z",type:"depends_on"}]}]})).toMatchObject({ok:false,error:{code:"relation_cycle"}});
  ...
  it("binds one repository request id to one operation and canonical payload",()=>{
    ...
    const input={idempotencyKey:"business-a",specs:[spec("request-bound")]};
    const first=dispatch.dispatch("kb.spec.apply",request,input);
    expect(first).toMatchObject({ok:true});
    expect(dispatch.dispatch("kb.spec.apply",{...request,now:"2026-07-13T12:05:00.000Z"},input)).toEqual(first);
    expect(dispatch.dispatch("kb.spec.apply",request,{...input,idempotencyKey:"business-b",specs:[spec("changed")]})).toMatchObject({ok:false,error:{code:"idempotency_conflict"}});
    expect(dispatch.dispatch("kb.status",request,{})).toMatchObject({ok:false,error:{code:"idempotency_conflict"}});
    expect(dispatch.dispatch("kb.spec.apply",{...request,requestId:"repo-request-2"},{idempotencyKey:"business-b",specs:[spec("changed")]})).toMatchObject({ok:true});
  });
  ...
  it("persists and replays typed handler failures without partial side effects",()=>{
    service.applySpecBatch(1,{idempotencyKey:"failure-seed",specs:[spec("failure-old"),spec("failure-new")]},ctx);
    service.mutate(1,"deprecate",{specId:"failure-new",idempotencyKey:"deprecate-new"},ctx);
    const request={...ctx,requestId:"failed-request"};
    const input={specId:"failure-old",replacementSpecId:"failure-new",idempotencyKey:"failed-supersede"};
    const first=dispatch.dispatch("kb.supersede",request,input);
    expect(first).toMatchObject({ok:false,error:{code:"replacement_not_active"}});
    ...
    expect(service.getSpec(1,"failure-new").state).toBe("deprecated");
  ...
  it("validates direct service inputs and mutation context before database work",()=>{
    ...
    invalid(()=>service.applySpecBatch(1,{idempotencyKey:"direct",specs:[spec("direct")]},{...ctx,now:"yesterday"}));
    invalid(()=>service.mutate(1,"mark_stale",{specId:"direct",idempotencyKey:"stale"},{...ctx,requestId:" "}));
    invalid(()=>service.applySpecBatch(0,{idempotencyKey:"spoofed-repo",specs:[spec("spoofed-repo")]},{...ctx,repoId:1} as OperationContext));
  ...
  it("returns honest bounded search pagination after ranking the complete filtered set",()=>{service.applySpecBatch(1,{idempotencyKey:"ranking",specs:[{...spec("a-early","alpha"),detail:"none",anchors:[{file:"src/auth/a.ts"}]},{...spec("m-detail","unrelated"),detail:"beta",anchors:[{file:"src/auth/m.ts"}]},{...spec("z-best","alpha beta"),anchors:[{file:"src/auth/z.ts"},{file:"src/shared/z.ts"}]}]},ctx);expect(service.searchSpecs(1,{query:"alpha beta",includeDrafts:true,limit:1})).toMatchObject({items:[{id:"z-best"}],count:1,total:3,limit:1,offset:0,hasMore:true,truncated:true});expect(service.searchSpecs(1,{query:"alpha beta",includeDrafts:true,offset:2})).toMatchObject({items:[{id:"m-detail"}],count:1,total:3,limit:50,offset:2,hasMore:false,truncated:false});expect(service.searchSpecs(1,{paths:["src/auth","src/shared"],includeDrafts:true,limit:1})).toMatchObject({items:[{id:"z-best"}],total:3,hasMore:true});});

  it("uses direct exact reads and bounded SQL pagination beyond 200 rows",()=>{for(let batch=0;batch<3;batch++){service.applySpecBatch(1,{idempotencyKey:`bulk-${batch}`,specs:Array.from({length:100},(_,i)=>({...spec(`bulk-${batch*100+i}`),featureId:undefined,anchors:[]}))},{...ctx,requestId:`bulk-${batch}`});}expect(service.getSpec(1,"bulk-299").id).toBe("bulk-299");const spy=vi.spyOn(db,"prepare");const page=service.searchSpecs(1,{includeDrafts:true,limit:3,offset:250});expect(page.items).toHaveLength(3);expect(page).toMatchObject({count:3,total:300,limit:3,offset:250,hasMore:true,truncated:true});const searchSql=spy.mock.calls.map(x=>String(x[0])).find(x=>x.includes("FROM kb_specs s")&&x.includes("LIMIT ? OFFSET ?"));expect(searchSql).toBeTruthy();expect(spy.mock.calls.filter(x=>String(x[0]).includes("kb_spec_current_anchors")).length).toBeLessThanOrEqual(1);spy.mockRestore();expect(service.review(1,{kinds:["unplaced"],limit:25,offset:225})).toMatchObject({total:300,limit:25,offset:225});expect(service.review(1,{kinds:["unplaced"],limit:25,offset:225}).items).toHaveLength(25);expect(service.getFeature(1,"auth").id).toBe("auth");});

  it("review excludes terminal lifecycle states from unplaced and low-confidence queues",()=>{service.applySpecBatch(1,{idempotencyKey:"review-lifecycle",specs:[{...spec("review-active"),featureId:undefined,evidence:[{...evidence[0]!,confidence:.4}]},{...spec("review-deprecated"),featureId:undefined,evidence:[{...evidence[0]!,confidence:.4}]}]},ctx);service.mutate(1,"deprecate",{specId:"review-deprecated",idempotencyKey:"review-deprecate"},ctx);const review=service.review(1);expect(review.items.map(x=>x.specId)).toContain("review-active");expect(review.items.map(x=>x.specId)).not.toContain("review-deprecated");});
  ...
  it("serializes genuine two-worker idempotency races across SQLite connections",async()=>{
    service.applySpecBatch(1,{idempotencyKey:"race-failure-seed",specs:[spec("race-failure-existing")]},ctx);
    db.close();
    ...
    const same={idempotencyKey:"race-same",specs:[spec("race-same-spec")]};
    ...
    const diff=await runPair([{idempotencyKey:"race-diff",specs:[spec("race-a")]},{idempotencyKey:"race-diff",specs:[spec("race-b")]}],i=>`diff-${i}`) as Array<{ok:boolean;error?:{code:string}}>;
    expect(diff.filter(x=>x.ok)).toHaveLength(1);expect(diff.find(x=>!x.ok)?.error?.code).toBe("idempotency_conflict");
    const requestRace=await runPair([{idempotencyKey:"request-race-a",specs:[spec("request-race-a")]},{idempotencyKey:"request-race-b",specs:[spec("request-race-b")]}],()=>"shared-request") as Array<{ok:boolean;error?:{code:string}}>;
    expect(requestRace.filter(x=>x.ok)).toHaveLength(1);
    expect(requestRace.find(x=>!x.ok)?.error?.code).toBe("idempotency_conflict");
    const failedInput={idempotencyKey:"race-failure",specs:[spec("race-failure-existing")]};
