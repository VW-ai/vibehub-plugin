(() => {
  ...
  const rooms = {
    workbench: {
      ...
      drift: [["✓", "All 8 anchored files match alignment hashes"], ["5e54b3a", "Last aligned commit"]],
    },
    product: {
      ...
      drift: [["⎇", "1 changed · docs/LOCAL_GRAPH_DESIGN.md"], ["+", "6 added · docs/proposals/**"], ["Action", "Review Context, then align or mark stale"]],
    },
    knowledge: {
      ...
      drift: [["✓", "All anchored files match alignment hashes"]],
    },
    "ticket-lifecycle": {
      ...
      drift: [["✓", "All 13 anchored files match alignment hashes"]],
    },
  };
  ...
  function render() {
    ...
    state.textContent = room.state === "FRESH" ? "✓ FRESH" : "⎇ DRIFTED";
    state.className = `state-label ${room.state === "FRESH" ? "fresh-label" : "drift-label"}`;
    const rows = view === "context" ? room.context.map(([type, label]) => ["◇", label, type])
      : view === "tickets" ? room.ticketsList.map((label) => ["□", label, "consuming Ticket"])
        : room.drift.map(([icon, label]) => [icon, label, room.state]);
    content.innerHTML = rows.map(([icon, label, meta]) => `<div class="list-row"><span class="list-icon">${icon}</span><span><strong>${label}</strong><small>${meta}</small></span><span>›</span></div>`).join("");
  }
  ...
  document.querySelector("#emptyToggle").addEventListener("click", () => {
    const makeEmpty = !detail.hidden;
    detail.hidden = makeEmpty; empty.hidden = !makeEmpty;
    if (makeEmpty) {
      selected = null;
      for (const peer of document.querySelectorAll("[data-room]")) peer.setAttribute("aria-selected", "false");
    } else {
      selected = "workbench";
      document.querySelector('[data-room="workbench"]').setAttribute("aria-selected", "true"); render();
    }
  });
  render();
})();
