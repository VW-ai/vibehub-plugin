...
const assetDirectory = join(scriptDirectory, "..", "apps", "harness-prototype");
...
const server = createServer(async (request, response) => {
  ...
  if (url.pathname === "/health") {
    ...
    response.end(request.method === "HEAD" ? undefined : JSON.stringify({ ok: true, prototype: true, durableState: false }));
    return;
  }
...
server.on("error", (error) => {
  process.stderr.write(`Unable to start the local prototype host: ${error.code ?? error.message}\n`);
  process.exitCode = 1;
});
...
server.listen(requestedPort, "127.0.0.1", () => {
  ...
  process.stdout.write(`${argumentsList.includes("--json") ? JSON.stringify(envelope) : `VibeHub Harness prototype: ${url}`}\n`);
