...
test("Codex light and dark primitives share one responsive accessible shell", async () => {
  ...
  for (const behavior of ["narrow drawer closes", "wide sidebar collapses without trapping focus", "search traps forward Tab", "request draft and focus survive reconciliation", "request draft survives an intentional route change", "streaming never replaces a selected transcript entry", "releasing the selection reconciles the held entry", "switching Thread does not leak Composer state", "Quote serializes exact source identity into the Turn input", "replayed quote renders its durable source identity", "Composer growth stops at the CSS ceiling", "active Turn exposes coherent Queue and Stop", "submission during a live Turn queues by default through queueTurn", "Alt\\+Enter steers the exact live Turn instead of queueing", "queued rows edit, delete and steer through the host queue actions", "an interrupted queue stays paused until an explicit Resume", "queuedStarted moves the follow-up into the transcript as its own Turn", "terminal reconciliation re-enables Fork", "Fork opens returned lineage", "Fork navigation updates the Thread deep link", "the raw Fork-of UUID line is gone and an unlisted source renders the dashed non-navigable state naming the id", "a fork's heading renders the navigable production source chip with the derived shared-Turn note", "the source chip opens the source on the chat route and the source heading lists each fork by title", "Bring back to source lands the labeled quote in the source composer draft, navigates with the draft visible and sends nothing", "the explicit send is a single turn/start whose input carries the quote with the fork's exact identity", "the sent message replays the quote with its fork identity", "Fork from here creates a point fork whose transcript ends at the chosen Turn and opens it with its source chip", "mounted timeline discloses its bound", "deferred mode and realtime controls make no contrary claim", "dark theme reaches overlay siblings", "page has no horizontal overflow", "Turn posture is internally coherent", "terminal mixed fixture makes no false live claim"]) assert.match(guard, new RegExp(behavior));
