...
test("the product view remains the faithful typed Ticket Workbench", async () => {
  ...
  assert.match(page, /data-edge-from=\{edge\.from\} data-edge-to=\{edge\.to\}/);
  assert.match(page, /function buildWorkbenchEdgeRoute/);
  assert.match(page, /data-ticket-id=\{ticket\.id\}/);
  assert.match(page, /ref=\{mapRef\} className="public-canvas-map"/);
  assert.match(page, /new ResizeObserver\(measureRoutes\)/);
  assert.match(page, /getBoundingClientRect\(\)/);
  assert.match(page, /source\.right - map\.left/);
  assert.match(page, /target\.left - map\.left/);
  ...
  assert.doesNotMatch(page, /Vibe task app|demo\/dark-mode|GUIDED FIXTURE|public-causal-edge|viewBox="0 0 1000 650"|path: "M 340/);
