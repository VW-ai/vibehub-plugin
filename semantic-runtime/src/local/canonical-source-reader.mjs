...
export class CanonicalSourceReader {
  ...
  constructor({ store, authority, repository_path, execution, registration_id, selection }) {
    ...
    this.#ingress = new DurableIngress({ store, authority }); this.#graph = new LocalGraphStore({ store, authority }); this.#feed = new SourceInvalidationFeed({ store, authority });
  }
  #call(operation) {
  ...
  #read(context, at, address) {
    ...
    const selected = this.#graph.resolve(context, { at, address });
    ...
    const head = this.#graph.getHead(context, { generation_id: at.generation_id });
  ...
  refresh(context, options) {
    return this.#call(() => {
      ...
      const prior = this.#graph.getReceipt(context, { generation_id: request.expected_graph.generation_id, idempotency_key: commandKey });
      if (prior) {
      ...
      const result = this.#graph.mutate(context, { epoch: request.epoch, publisher_ref: request.publisher_ref.publisher_ref,
