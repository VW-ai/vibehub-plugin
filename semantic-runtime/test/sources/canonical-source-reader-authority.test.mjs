...
import { fixture, register, capture, initialize, assertion, mutation, rows, hashText, SCOPE, ACTIONS, git } from '../support/graph-store-fixture.mjs';
...
test('a second process moving the fence after initial inspection prevents every Graph write', t => {
  ...
  f.store.readSnapshot = (context, operation) => {
    ...
    if (!moved) {
      moved = true;
      const child = spawnSync(process.execPath, [new URL('../support/canonical-fence-process.mjs', import.meta.url).pathname,
