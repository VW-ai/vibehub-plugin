...
test('persisted JEV smoke reopens approved bytes, deduplicates intake and sends only minimal synthetic input', async () => {
  let calls = 0;
  ...
  assert.equal(calls, 8); assert.equal(report.completed, 8); assert.equal(report.matched, 8);
  assert.deepEqual(report.ingress, { stored: 8, reopened: true, deduplicated: 8, materialized: 8,
    pending_intents: 8, semantic_processing_claimed: false });
