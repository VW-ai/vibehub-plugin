...
test("pausing the running side (keyboard through the menu) → PAUSING feedback naming both sides", async ({ page }) => {
  ...
  await expect(band.locator(".demo")).toBeVisible();
...
test("confirming ignore → IGNORED feedback scoped to THIS pair; evidence stays readable", async ({ page }) => {
  ...
  await expect(band.locator(".demo")).toHaveAttribute("data-tip", /no live session/);
...
test("Re-run toggles an honest demo note — no fake progress, verdict untouched", async ({ page }) => {
  ...
  await expect(note).toContainText("Demo — nothing ran");
