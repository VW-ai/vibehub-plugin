 ...
 * dynamized from static/menubar-s1.html (approved S1+S2, merged). A separate
 * demo render path like InstallScreen: `?menubar=<variant>` renders a generic
 * desktop + menubar strip + the Vibehub item + its dropdown INSTEAD of the
 * map window. Everything product-visible comes from one MapFixture through
 * the pure rollup (deriveMenubar) — the menubar can never disagree with the
 * map (S3 design; no menubar fixture shape exists).
 *
 * S5 interactions:
 *  - item click toggles the dropdown (starts OPEN on load — the dropdown IS
 *    the demo subject, and parity shots need it; real app starts closed);
 ...
 *    each button carries lives in its tooltip (demo has no main window to
...
const STAND_IN_TIP = "decorative stand-in in this demo";
...
export function MenubarScreen({
  ...
  const s = useMemo(() => deriveMenubar(fixture), [fixture]);

  // The dropdown IS the demo subject → starts open (parity with the static).
  ...
  return (
    <div className="mbdesk">
      ...
      {showSwitcher && variantNames.length > 1 && (
        <select
          className="fixture-switch mb-switch"
          aria-label="Demo menubar variant"
          value={activeVariant}
