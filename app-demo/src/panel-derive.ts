...
import type { MapFixture, ScopeDeclaration, Task, TaskState } from "./types";
  ...
  switch (e.type) {
    ...
    case "user_injection":
      return e.mode === "inject"
        ? `${c} — injected from this panel, without interrupting`
        : `${c} — sent from this panel after stopping the agent`;
    case "question":
...
function scopeName(s: ScopeDeclaration, map: MapFixture): string {
  return map.territories.find((t) => t.id === s.territoryId)?.name ?? s.label;
}

export function panelScopeChips(task: Task, map: MapFixture): PanelChipView[] {
...
export function twistView(p: TaskPanelFixture): TwistView | null {
...
export function deckTextareaTip(state: TaskState): string {
  return state === "waiting"
    ? "Free-form reply. Right now the agent is waiting, so either mode delivers immediately."
    : "Free-form message. Delivery follows the selected mode.";
}
