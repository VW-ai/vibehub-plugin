/**
 * menubar-derive.ts (m5, S3) — the pure rollup MapFixture → MenubarSummary.
 *
 * HARD RULE (same as derive.ts): zero hardcoded CONTENT — everything shown
 * is fixture data or a mechanical join of it. The only literals are UI copy
 * explaining state semantics (chrome, honest per LOOP.md guideline 4).
 *
 * Encoded S1 open questions (notes/menubar.md):
 *   1. No menubar fixture shape — this file IS the reuse.
 ...
 */
import type { Conflict, MapFixture, Task } from "./types";
import type {
  MenubarBadgeView,
  MenubarFreshView,
  MenubarStatView,
...
function waitingRow(t: Task, fx: MapFixture): NeedsYouRowView {
...
function conflictSubject(c: Conflict, fx: MapFixture): string {
...
function conflictRow(c: Conflict, fx: MapFixture): NeedsYouRowView {
...
function needsYou(fx: MapFixture): NeedsYouView {
...
function fetchAge(fx: MapFixture): string | null {
  return fx.sync.lastFetchAt === null
    ? null
    : relAge(fx.sync.lastFetchAt, fx.capturedAt);
}

function fresh(fx: MapFixture): MenubarFreshView {
...
function staleNote(fx: MapFixture): StaleNoteView | null {
 ...
 * no-timezone rule as clockTime — demos render identically everywhere).
 */
...
export function deriveMenubar(fx: MapFixture): MenubarSummary {
